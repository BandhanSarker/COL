...
#'
#'COL aims to identify putatively functional back-splicing based on the molecular error hypothesis.
#'COL combined (1) substantially higher back-splicing rates than expected from the total splicing amounts;
#' (2) conserved splicing motifs between different species;
#' (3) high back-splicing levels to identify functional back-splicing.
#' COL has been proved to be very effective on identifying shared back-splicing junctions and experimentally functional back-splicing junctions.
#'
...
#'@title genome
#'@name genome
#'@param  genome.UCSC DNA sequence (FASTA) Hgenome:BSgenome.Hsapiens.UCSC.hg38; Mgenome: BSgenome.Mmusculus.UCSC.mm10
#'@param genomeOther other DNA sequence (FASTA) instead of Homo sapiens; if user use independent genome FASTA.
#'@export
...
############
#'@export
outlier<-function(cirRNA,STAR_output){
  BCPAP.rep.1<-cirRNA
  ...
  huCsIndex1 = huCooksd > 4*mean(huCooksd, na.rm=T)
  huCsIndex2 = (huCooksd > 4*mean(huCooksd, na.rm=T))& (residuals(huMod)>0)
  huCsIndex2 = which(huCsIndex2==TRUE)
  huCsIndex3 = (huCooksd > 4*mean(huCooksd, na.rm=T))& (residuals(huMod)<0)
...
cmotif<-function(genome.UCSC="Hgenome",genomeOther,cRNA){
  ...
  output<-data.frame()
  for (i in 1:length(unique(cRNA$chr))){
    ######### Forward
    genI<-gen[[unique(cRNA$chr)[i]]]
    chrF<-unique(cRNA$chr)[i]
    cRNAI<-cRNA[cRNA$chr %in% chrF & cRNA$strand %in% "+",]
    outtF<-data.frame()
    for (j in 1:dim(cRNAI)[1]){
      motifF<-paste(substr(as.character(genI),as.numeric(cRNAI[j,]$start)-2,as.numeric(cRNAI[j,]$start)-1),
                    substr(as.character(genI),as.numeric(cRNAI[j,]$end)+1,as.numeric(cRNAI[j,]$end)+2),sep="")
      outF<-cbind(cRNAI[j,],motif=motifF)
      outtF<-rbind(outtF,outF)
    }
    #####Reverse
    cRNAR<-cRNA[cRNA$chr %in% chrF & cRNA$strand %in% "-",]
    outtR<-data.frame()
    for (j in 1:dim(cRNAR)[1]){
      motifR<-paste(substr(as.character(genI),as.numeric(cRNAR[j,]$start)-2,as.numeric(cRNAR[j,]$start)-1),
                    substr(as.character(genI),as.numeric(cRNAR[j,]$end)+1,as.numeric(cRNAR[j,]$end)+2),sep="")
      outR<-cbind(cRNAR[j,],motif=as.character(Biostrings::reverseComplement(Biostrings::DNAString(motifR))))
      outtR<-rbind(outtR,outR)
    }
    outchr<-rbind(outtF,outtR)
    output<-rbind(output,outchr)
  }
  #######
  ###save the result
  return(output)
...
#'@name liftOVER
#'@description
#'liftOVER function build based on the UCSC liftover tools for lifting features for specific region rather than
#'interval region from one genome to other.
...
#'@name conserve
#'@description
#'conserve function  build to identify putatively functional back-splicing based on the molecular error hypothesis.COL combined (1) substantially higher back-splicing rates than expected from the total splicing amounts; (2) conserved splicing motifs between different species; (3) high back-splicing levels to identify functional back-splicing.
#'@param cirRNA cirularRNA data
#'@param STAR_output output file from STAR
...
#'@param chainObject chainObject usually downloaded from https://hgdownload.soe.ucsc.edu/goldenPath/hg38/liftOver/
